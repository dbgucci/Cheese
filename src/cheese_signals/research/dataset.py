...
# Timeframe suffixes that exporters append to a pair name. "EURUSD_otc_M1"
# and "EURUSD_otc" are the same instrument recorded by two different tools,
# and failing to merge them leaves both piles too small to study.
_TIMEFRAME_SUFFIX = re.compile(
    r"[_\-. ]?(m1|m5|m15|m30|h1|h4|d1|1m|5m|15m|30m|1h|4h|1d|60|1min)$",
    re.IGNORECASE,
)


def canonical_asset(name: str) -> str:
    """Merge key for an asset name, ignoring timeframe suffix and punctuation.

    ``EURUSD_otc_M1`` -> ``EURUSDOTC``, matching ``EURUSD_otc``.

    The OTC marker is deliberately *kept*: ``EURUSD`` and ``EURUSD_otc`` are
    different instruments -- one is a real market, the other is generated by
    the broker -- and merging them would be a serious error.
    """
    out = name.strip()
    previous = None
    while previous != out:  # strip repeated suffixes, e.g. "EURUSD_otc_M1_1m"
        previous = out
        out = _TIMEFRAME_SUFFIX.sub("", out)
    return re.sub(r"[^A-Z0-9]", "", out.upper())


def _preferred_name(a: str, b: str) -> str:
    """Pick the tidier display name for a merged asset (the shorter one)."""
...
def collect(
    ...
    for stream in streams:
        for history in stream:
            ...
            key = canonical_asset(history.asset)
            ...
            merged[key] = AssetHistory(
                asset=_preferred_name(existing.asset, history.asset),
                candles=combined,
                source=sources,
            )
