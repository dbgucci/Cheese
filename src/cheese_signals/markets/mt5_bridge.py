...
**Order placement deliberately lives elsewhere**, in ``mt5_trader.py``. It was
originally in this file, and that made a claim the signals app wanted to make
untrue: excluding the trading modules from that build left ``MT5Trader`` in the
binary anyway, because it shared a module with the feed the signals app needs.
CI greps the built exe for MetaTrader's order-sending call and failed,
correctly. Keeping reading and writing in separate modules is what lets a build
contain one without the other -- and this sentence avoids naming that call
literally, because a docstring mentioning it is enough to trip the same check.
"""
...
class MT5Feed:
    ...
    _TIMEFRAMES = {M1: "TIMEFRAME_M1", M5: "TIMEFRAME_M5",
                   M15: "TIMEFRAME_M15", H1: "TIMEFRAME_H1", D1: "TIMEFRAME_D1"}
    ...
    def server_time(self, symbol: str) -> datetime:      # pragma: no cover
        """The broker's clock, read from a live tick.

        There is no API for "what time does the server think it is", so the
        timestamp on the most recent tick stands in for it. Read as UTC --
        which it is not, and that discrepancy is exactly the measurement:
        ``clock.measure_server_offset`` turns it into an offset.
        """
        if not self._mt5.symbol_select(symbol, True):
            raise KeyError(f"{symbol} is not available on this account")
        tick = self._mt5.symbol_info_tick(symbol)
        if tick is None:
            raise RuntimeError(f"no live tick for {symbol}, so the server clock "
